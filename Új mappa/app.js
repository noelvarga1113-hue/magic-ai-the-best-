...
(function () {
  ...
  function findAnswer(input) {
    ...
    if (bestKB && bestKBScore >= MATCH_THRESHOLD) {
      ...
      return { text: answers[Math.floor(Math.random() * answers.length)], source: "kb", score: bestKBScore, entry: bestKB };
    }
  ...
  function findRelatedFact(input, mainEntry, mainText) {
    ...
    var best = null, bestScore = 0;
    for (var k = 0; k < kb.length; k++) {
      if (kb[k] === mainEntry) continue;
      // Az azonos sablonból generált bejegyzés (pl. másik kémiai elem)
      // nem igazi érdekesség, azt nem ajánljuk kapcsolódóként
      if (mainEntry && mainEntry.t && kb[k].t === mainEntry.t) continue;
      for (var p = 0; p < kb[k].q.length; p++) {
        var s = scoreMatch(input, kb[k].q[p]);
        if (s > bestScore) { bestScore = s; best = kb[k]; }
      }
    }
    if (best && bestScore >= 0.55 && best.a[0] !== mainText) return best.a[0];
    return null;
  }
  ...
  function maybeAttachYouTube(row, question, source) {
    if (!row) return;
    if (source !== "kb" && source !== "wiki" && source !== "wiki-en" && source !== "ddg") return;
  ...
  function addGreeting() {
    ...
    var hi = nick ? ("Szia, **" + nick + "**! 👋 ") : "Szia! 👋 ";
    var seenVersion = null;
    try { seenVersion = localStorage.getItem(VERSION_KEY); } catch (e) {}
    if (seenVersion !== APP_VERSION) {
      addBotMessage(
        hi + "🎉 **Itt a Magic AI 2.1!** 🪄\n\n" +
        "Ezeket adtuk most hozzá:\n" +
        "• 🔐 **Fiókok** – mostantól regisztrálsz és bejelentkezel (teljes név + jelszó), így a beszélgetéseid a sajátjaid maradnak.\n" +
        "• 📜 **Előző chatek** – a felső sávban visszatérhetsz a korábbi beszélgetéseidhez, és ugyanúgy írhatsz beléjük tovább.\n" +
        "• 👥 **Group** – közös, online csoportos chat: add meg a neved, e-mailed és egy csoport kódot, és a barátaiddal egy közös chatbe írhattok – akár külön gépekről is!\n\n" +
        "Minden eddigi tudásom és funkcióm a helyén – kérdezz bátran bármit! 😊",
        {}
      );
    } else {
      addBotMessage(
        hi + "Én vagyok a **Magic AI** – a saját fejlesztésű mesterséges intelligenciád.\n" +
        "Kérdezz bátran! Megmondom az **időjárást** ⛅, elhozom a **friss híreket** 📰, **weboldalakat ajánlok** 🔗, és ha helyben nem tudom, **utánanézek az interneten** 🌐. " +
        "A felső sávban ott a **📜 Előző chatek** és a **👥 Group** csoportos chat is. Jó beszélgetést! 🪄",
        {}
      );
    }
  }
  ...
  function addBotMessage(text, opts) {
    ...
    if (opts.source === "learned") {
      ...
      tag = '<span class="source-tag">✨ helyi AI-modell (béta)</span>';
    }
  ...
  function handleSend() {
    ...
    setTimeout(function () {
      ...
      // a VALÓDI személyleírást a Wikipédiáról hozzuk (a neten alátámasztva).
      ...
      // Helyben nincs válasz. Ha a böngészős AI-modell betöltve+engedélyezve van,
      // ŐVELE válaszolunk a csevegésben (és nem keresünk a neten). Egyébként web-keresés.
  ...
  function chatAskAI(text) {
    var L = window.MagicLLM;
    hideTyping();
    islandShow("✨", "A helyi AI gondolkodik…");
    var row = addBotMessage("✨ …", { source: "ai", feedback: true, question: text, silent: true });
    var bubble = row.querySelector(".bot-bubble");
    L.run(L.CHAT_SYSTEM, text, function (full) {
      if (bubble) bubble.innerHTML = formatText(full) + ' <span class="hw-ai-caret">▍</span>';
      scrollToBottom();
    }, { temperature: 0.5 }).then(function (full) {
  ...
  function webSearchFallback(text, forceWeb) {
    ...
    searchWikipedia(text, "hu")
      .then(function (w) { return { kind: "wiki", title: w.title, text: w.text, url: w.url }; })
      .catch(function () {
        return searchWikipedia(text, "en").then(function (w) {
          return { kind: "wiki-en", title: w.title, text: w.text, url: w.url };
        });
      })
      .catch(function () {
        return searchDuckDuckGo(text).then(function (d) {
    ...
    return new Promise(function (resolve) {
      ...
      function viaWeb() {
        searchWikipedia(text, "hu")
          ...
          .catch(function () {
            return searchDuckDuckGo(text).then(function (dd) {
              return { kind: "ddg", title: dd.title, text: dd.text, url: dd.url };
            });
          })
          .then(function (r) {
  ...
  function updateStats() {
    $("stat-kb").textContent = (window.MAGIC_KB || []).length;
  ...
  // A felismert feladat elküldése az AI-modellnek a TÍPUSHOZ illő utasítással (streamelt)
  function hwSolveAI(text, cls) {
    ...
    if (cls && cls.type === "fogalmazás") {
      runAIStream(L.writeComposition.bind(L, text), "✨ Az AI fogalmazást ír… (" + cls.label + ")",
        "📸 **Házi feladat – AI fogalmazás** ✍️\n\n", null, text, cls);
      return;
    }

    // MATEK → a PONTOS helyi megoldót használjuk (mindig helyes, lépésről lépésre).
    //         A kis AI-modell a számolást gyakran elrontja, ezért matekot NEM bízunk rá,
    //         ha a beépített motor meg tudja oldani. Csak ha NEM boldogul, próbálja az AI.
    if (cls && cls.type === "matek" && H) {
      var exact = null;
      try { exact = H.solveMath(text); } catch (e) {}
      if (exact) {
        hwRender([{ plain: true, a: exact, q: text }], "pontos megoldás 🧮");
        return;
      }
      // nincs pontos helyi megoldás → az AI megpróbálja (de jelezzük a bizonytalanságot)
    }

    // KÉRDÉS / tényszerű feladat → előbb a PONTOS beépített tudástár (offline, megbízható),
    // mert az konkrét tényt ad – a kis AI-modell könnyen melléfog. Csak ha a tudástár sem
    // tudja, akkor jön az AI.
    if (cls && cls.type === "kérdés") {
      var kb = findAnswer(text);
      if (kb) {
        hwRender([{ q: text.length > 80 ? text.slice(0, 80) + "…" : text, a: kb.text }], "tudástár 📚");
        return;
      }
    }

    // KÉRDÉS (tudástárban nincs) / megoldhatatlan matek → az AI válaszol
    var system = (cls && cls.system) || "";
    var head = "✨ Az AI válaszol… (" + (cls ? cls.label : "feladat") + ")";
    ...
    L.run(system, text, function (full) {
      ...
      addBotMessage(applyStyle("📸 **Házi feladat – AI válasz** (" + (cls ? cls.label : "feladat") + "):\n\n" + full), {});
    }).catch(function (err) {
  ...
  function setupAIPanel(ids) {
    ...
    if (select) {
      ...
      select.value = L.currentModel() || L.defaultModel();
  ...
  function setupGroupExtra() {
    // A csoportos chat MOST egy önálló ablakban nyílik (group.html).
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (window.MagicAuth && window.MagicAuth.current()) {
      enterApp();
    } else {
      showAuthScreen();
    }
