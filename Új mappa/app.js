...
(function () {
  ...
  // 3.0: nagyobb modellek (Expert 8B / Normal 7B / Fast 4B), nyelvenkénti
  // letöltés-élmény, külön Házi feladat modul kép-értő (vision) modellel.
  var APP_VERSION = "3.0";
  ...
  function stopSpeaking() {
    speakSession++;
    if (window.MagicVoice) window.MagicVoice.stop();
  ...
  function speakText(text) {
    ...
    // A TELJES AI EGY közös, saját hangon szól: window.MagicVoice (voice.js).
    // Neurális (saját, egyedi) hang, tartalékként a legszebb rendszerhang.
    var MV = window.MagicVoice;
    if (MV) {
      try { MV.setLang((window.MagicLLM && MagicLLM.getLang && MagicLLM.getLang()) || "hu"); } catch (e) {}
      MV.speak(clean, {
        onStart: startTalkingFace,
        onBoundary: mouthPulse,
        onEnd: function () { if (session !== speakSession) return; stopTalkingFace(); oceanSink(); }
      });
      return true;
    }
  ...
  function addGreeting() {
    ...
    if (lang === "hu" && seenVersion !== APP_VERSION) {
      addBotMessage(
        hi + "🎉 **Megérkezett a Magic AI 3.0!** 🪄\n\n" +
        "Mi újult meg most:\n" +
        "• 🧠 **Nagyobb, okosabb modellek:** Expert (8B), Normal (7B) és Fast (4B) – több tudás, jobb többnyelvűség.\n" +
        "• 🌐 **Nyelvenkénti letöltés:** minden nyelvet külön töltesz le, és a modell a beállított nyelven beszél.\n" +
        "• 📸 **Megújult Házi feladat modul:** tölts fel egy **fotót** a feladatról – a **kép-értő modul a szöveget ÉS az ábrákat, rajzokat is** elolvassa és megoldja.\n" +
        "• 👥 A **Group** csoportos chat a kiegyensúlyozott **Normal** modellel válaszol; a béta feliratok lekerültek.\n\n" +
  ...
  var I18N = {
    hu: {
      ...
      extra_group: "👥 Group", extra_homework: "📸 Házi feladat", extra_game: "🎮 Game", extra_aimodel: "🤖 AI-modell",
    ...
    en: {
      ...
      extra_group: "👥 Group", extra_homework: "📸 Homework", extra_aimodel: "🤖 AI model",
    ...
    de: {
      ...
      extra_group: "👥 Gruppe", extra_homework: "📸 Hausaufgaben", extra_aimodel: "🤖 KI-Modell",
    ...
    ru: {
      ...
      extra_group: "👥 Группа", extra_homework: "📸 Домашка", extra_aimodel: "🤖 ИИ-модель",
    ...
    pt: {
      ...
      extra_group: "👥 Grupo", extra_homework: "📸 Trabalho de casa", extra_aimodel: "🤖 Modelo de IA",
    ...
    es: {
      ...
      extra_group: "👥 Grupo", extra_homework: "📸 Deberes", extra_aimodel: "🤖 Modelo de IA",
      stat_model: "Modelo de IA", stat_learned: "aprendido",
    ...
    fr: {
      ...
      extra_group: "👥 Groupe", extra_homework: "📸 Devoirs", extra_aimodel: "🤖 Modèle d'IA",
    ...
    it: {
      ...
      extra_group: "👥 Gruppo", extra_homework: "📸 Compiti", extra_aimodel: "🤖 Modello IA",
  ...
  function setupLang() {
    ...
    sel.addEventListener("change", function () {
      ...
      applyLang(code);
      // A NYELVENKÉNTI letöltés-élmény: a panelek gombjai/zászlósorai az új nyelvre álljanak
      // (ha az adott nyelv még nincs „letöltve" a modellhez, a gomb újra letöltést kínál).
      if (typeof refreshLoadBtns === "function") refreshLoadBtns();
      if (visionUI && visionUI.refresh) visionUI.refresh();
  ...
  // ENERGIA-OPTIMALIZÁLÁS – AUTOMATIKUS NYUGALMI MÓD
  // A díszítő animációk (elmosott aurora, hullámzó tenger) folyamatosan dolgoztatnák a
  // GPU-t. A `body.calm` osztály megállítja őket (lásd style.css), amikor nincs rájuk
  // szükség: ha a böngészőfül háttérbe kerül, VAGY ha a felhasználó ~10 mp-ig tétlen.
  // Bármilyen valódi interakcióra azonnal feléled. Amíg az aurora áll, a böngésző a
  // backdrop-filtert is gyorsítótárazza → a videókártya pihen (ez szünteti meg azt,
  // hogy az app tétlenül is 100%-on járatja a GPU-t). Generálás közben (a felhasználó
  // csak nézi a választ) szintén bekapcsol → a GPU teljes ereje a modellé.
  var calmIdle = false;
  var calmIdleTimer = null;
  var CALM_IDLE_MS = 10000;

  function recomputeCalm() {
    document.body.classList.toggle("calm", document.hidden || calmIdle);
  }
  function calmGoIdle() { calmIdle = true; recomputeCalm(); }
  function calmWake() {
    if (calmIdle) { calmIdle = false; recomputeCalm(); }
    clearTimeout(calmIdleTimer);
    calmIdleTimer = setTimeout(calmGoIdle, CALM_IDLE_MS);
  }
  function setupPowerSaver() {
    document.addEventListener("visibilitychange", recomputeCalm);
    // A 'scroll' SZÁNDÉKOSAN kimarad: a streamelt válasz automatikus görgetése
    // scroll-eseményt szülne, ami feleslegesen ébresztgetné az aurorát. Csak valódi
    // felhasználói interakcióra ébredünk (egér/érintés/billentyű/görgő).
    ["pointermove", "pointerdown", "keydown", "wheel", "touchstart"].forEach(function (ev) {
      window.addEventListener(ev, calmWake, { passive: true });
    });
    calmWake(); // elindítja a tétlenségi időzítőt
  }
  ...
  function setupSettings() {
    ...
    // Energiatakarékos mód kapcsoló – ALAPBÓL BE (a felhasználó kérése: minél
    // kisebb GPU-terhelés). Csak akkor van kikapcsolva, ha ő maga kapcsolta ki.
    var ecoToggle = $("eco-toggle");
    if (ecoToggle) {
      var ecoOn = localStorage.getItem(ECO_KEY) !== "0";
    ...
    // SAJÁT, EGYEDI (neurális) HANG – ugyanaz, mint a jósgömbben (voice.js /
    // window.MagicVoice). A felolvasás automatikusan ezen szól, ha le van
    // töltve; itt lehet előre letölteni, ki-/bekapcsolni és kipróbálni.
    var MV = window.MagicVoice;
    var nToggle = $("neural-toggle"), nLoad = $("neural-load"), nSample = $("neural-sample");
    var nBar = $("neural-bar"), nFill = $("neural-bar-fill"), nStatus = $("neural-status");
    if (MV && nToggle && nLoad) {
      var voiceLang = function () {
        try { return (window.MagicLLM && MagicLLM.getLang && MagicLLM.getLang()) || "hu"; }
        catch (e) { return "hu"; }
      };
      var refreshNeuralUI = function () {
        var lang = voiceLang();
        if (MV.neuralReady(lang)) {
          nLoad.textContent = "✅ Saját hang kész";
          nLoad.disabled = true;
          nStatus.textContent = "Ezen a saját hangon olvasom fel a válaszaimat.";
        } else if (MV.neuralDownloaded(lang)) {
          nLoad.textContent = "⬇️ Saját hang betöltése";
          nStatus.textContent = "💾 Korábban letöltve – az első megszólaláskor magától betöltődik.";
        } else {
          nLoad.textContent = "⬇️ Saját hang letöltése";
          nStatus.textContent = "";
        }
      };
      nToggle.checked = MV.neuralEnabled();
      nToggle.addEventListener("change", function () {
        MV.setNeuralEnabled(nToggle.checked);
        islandFlash("✨", nToggle.checked
          ? "Saját hang bekapcsolva!" : "Saját hang kikapcsolva – a rendszerhang szól.", 2200);
      });
      if (!MV.neuralSupported()) {
        nToggle.checked = false;
        nToggle.disabled = true;
        nLoad.disabled = true;
        if (nSample) nSample.disabled = true;
        nStatus.textContent = "Ezen a böngészőn sajnos nem támogatott.";
      } else {
        nLoad.addEventListener("click", function () {
          ...
          nBar.hidden = false;
          nFill.style.width = "0%";
          nStatus.textContent = "⬇️ Saját hang letöltése… (csak egyszer kell)";
          MV.loadNeural(lang, function (f) {
            nFill.style.width = Math.round((f || 0) * 100) + "%";
          }).then(function () {
            nBar.hidden = true;
            refreshNeuralUI();
            // rövid bemutatkozás az új hangon
            MV.setLang(lang);
            MV.sample("Szia! Mostantól ez a saját hangom!", lang);
          }).catch(function () {
            nBar.hidden = true;
            nLoad.disabled = false;
            nStatus.textContent = "❌ Nem sikerült a letöltés – nézd meg a netkapcsolatot, és próbáld újra.";
          });
        });
        if (nSample) nSample.addEventListener("click", function () {
          var lang = voiceLang();
          MV.setLang(lang);
          MV.sample("Szia! Így hangzik a Magic AI hangja.", lang);
        });
        refreshNeuralUI();
      }
    }
  ...
  // A feltöltött FOTÓ KÖZVETLEN megoldása a kép-értő (vision) modullal: a modell
  // maga olvassa el a képen lévő szöveget ÉS értelmezi az ábrákat/rajzokat, majd a
  // beállított rendszernyelven megoldja. Hiba esetén visszaesik az OCR-re (a kép
  // szövegét a mezőbe írja, hogy a szöveges úton megoldható legyen).
  function hwSolveImage(dataUrl) {
    var L = window.MagicLLM;
    var resEl = $("hw-result");
    if (resEl) {
      resEl.hidden = false;
      resEl.innerHTML = '<div class="hw-result-head">🖼️ Kép-értő megoldás (szöveg + ábrák)</div><div id="hw-ai-stream"></div>';
    }
    hwStatus("🖼️ A kép-értő modul olvassa a feladatot és az ábrákat…");
    var streamEl = $("hw-ai-stream");
    L.runVision(
      dataUrl,
      "Olvasd el pontosan a képen lévő házi feladatot – a szöveget (kézírást is) ÉS az ábrákat, rajzokat, koordináta-rendszereket, táblázatokat is. Állapítsd meg, mit kell megoldani és mi az elérni kívánt eredmény, majd oldd meg lépésről lépésre.",
      function (full) {
      ...
      addBotMessage(applyStyle("📸 **Házi feladat – kép-értő megoldás** 🖼️\n\n" + full), {});
    }).catch(function () {
      hwStatus("⚠️ A kép-értő modul most nem válaszolt – beolvasom a szöveget OCR-rel…");
      runOCR(dataUrl).then(function (t) {
        var H = window.MagicHomework;
        var clean = (H && H.normalizeOCR) ? H.normalizeOCR(t)
          : (t || "").replace(/[ \t]+/g, " ").replace(/\n{3,}/g, "\n\n").trim();
        var textEl = $("hw-text");
        if (textEl) textEl.value = clean;
        if (clean) hwStatus("✅ Beolvastam a szöveget – ellenőrizd, majd nyomd meg a Megoldás gombot.");
        else hwStatus("🙈 Nem találtam szöveget a képen. Próbáld élesebb fotóval, vagy gépeld be a feladatot.");
      }).catch(function () {
        hwStatus("😕 A kép olvasása most nem sikerült. Gépeld be a feladatot a mezőbe, és úgy is megoldom!");
      });
    });
  }
  ...
  var aiPanels = []; // { select, loadBtn, bar, fill, status, toggleWrap, langs }
  var visionUI = null; // a kép-értő (vision) modul panel-kezelője (setupVisionPanel állítja be)

  // Nyelv-kód → zászló + endonima rövid név (a NYELVENKÉNTI letöltés-élményhez)
  var LANG_META = {
    hu: { flag: "🇭🇺", name: "Magyar" },
  ...
  function langMeta(code) { return LANG_META[code] || { flag: "🌐", name: code }; }
  function langDisplay(code) { var m = langMeta(code); return m.flag + " " + m.name; }
  function modelSize(m) { return m ? (m.mb >= 1000 ? (m.mb / 1000).toFixed(1) + " GB" : m.mb + " MB") : ""; }

  // A „letöltött" nyelvek zászlósora egy adott modellhez (az aktuális nyelv kiemelve).
  function langsRowHtml(modelId) {
    var L = window.MagicLLM;
    var cur = getLang();
    var ready = (L.readyLangs ? L.readyLangs(modelId) : []);
    var html = '<span class="hw-ai-langs-label">📥 Letöltött nyelvek:</span> ';
    if (!ready.length) {
      html += '<span class="hw-ai-lang-empty">még egy sincs – tölts le egyet a beállított nyelven</span>';
    } else {
      html += ready.map(function (code) {
        return '<span class="hw-ai-lang' + (code === cur ? " current" : "") + '" title="' +
          langMeta(code).name + '">' + langMeta(code).flag + "</span>";
      }).join("");
    }
    if (L.langReady && !L.langReady(modelId, cur)) {
      html += ' <span class="hw-ai-lang-todo">' + langDisplay(cur) + " – még nincs letöltve</span>";
    }
    return html;
  }

  // A két szöveges panel (Házi + chat) nyelvsorának frissítése a kiválasztott modellhez
  function renderPanelLangs() {
    var L = window.MagicLLM;
    aiPanels.forEach(function (p) {
      if (!p.langs) return;
      var modelId = p.select ? p.select.value : L.defaultModel();
      p.langs.innerHTML = langsRowHtml(modelId);
    });
  }
  ...
  // A „Betöltés/Váltás" gombok feliratának frissítése a kiválasztott modell, a betöltött
  // modell ÉS az aktuális NYELV alapján (nyelvenkénti letöltés-élmény).
  function refreshLoadBtns() {
    var L = window.MagicLLM;
    var lang = getLang();
    var ld = langDisplay(lang);
    aiPanels.forEach(function (p) {
      if (!p.loadBtn) return;
      if (L.isLoading()) { p.loadBtn.textContent = "⏳ Letöltés…"; p.loadBtn.disabled = true; return; }
      var sel = p.select ? p.select.value : L.defaultModel();
      var cur = L.currentModel();
      var m = L.MODELS.filter(function (x) { return x.id === sel; })[0];
      if (cur && cur === sel) {
        if (L.langReady && L.langReady(sel, lang)) { p.loadBtn.textContent = "✅ " + ld + " – betöltve"; p.loadBtn.disabled = true; }
        else { p.loadBtn.textContent = "⬇️ " + ld + " letöltése"; p.loadBtn.disabled = false; }
      } else if (cur) {
        p.loadBtn.textContent = "🔄 Váltás: " + ld + " · " + (m ? m.tier : ""); p.loadBtn.disabled = false;
      } else {
        p.loadBtn.textContent = "⬇️ " + ld + " letöltése · " + modelSize(m); p.loadBtn.disabled = false;
      }
    });
    renderPanelLangs();
  }
  ...
  function doLoadModel(modelId, auto) {
    ...
    if (L.currentModel() === modelId) {
      // A modell már fut. Ha az AKTUÁLIS nyelv még nincs „letöltve" ehhez a modellhez,
      // gyors nyelv-aktiválás: a súlyok már megvannak, csak megjelöljük letöltöttként.
      var lang = getLang();
      if (L.markLangReady && L.langReady && !L.langReady(modelId, lang)) {
        L.markLangReady(modelId, lang);
        aiPanelStatus("✅ " + langDisplay(lang) + " letöltve – az AI ezen a nyelven válaszol. (a modell már megvolt, ezért gyors)");
      }
      refreshLoadBtns();
      return;
    }
  ...
  function setupAIPanel(ids) {
    ...
    var p = {
      ...
      status: status, toggleWrap: $(ids.toggleWrap), langs: $(ids.langs)
    };
    aiPanels.push(p);
  ...
  function setupAIPanels() {
    setupAIPanel({ panel: "hw-ai", select: "hw-ai-model", load: "hw-ai-load", bar: "hw-ai-bar", fill: "hw-ai-bar-fill", status: "hw-ai-status", toggleWrap: "hw-ai-toggle-wrap", langs: "hw-ai-langs" });
    setupAIPanel({ panel: "chat-ai", select: "chat-ai-model", load: "chat-ai-load", bar: "chat-ai-bar", fill: "chat-ai-bar-fill", status: "chat-ai-status", toggleWrap: "chat-ai-toggle-wrap", langs: "chat-ai-langs" });
    setupVisionPanel();
  ...
  // A kép-értő (vision) modul „letöltött nyelvek" sora
  function renderVisionLangs() {
    var L = window.MagicLLM;
    var el = $("hw-vision-langs");
    if (!el || !L.readyLangs) return;
    el.innerHTML = langsRowHtml(L.VISION_MODEL.id);
  }

  // KÜLÖN HÁZI FELADAT MODUL – kép-értő (vision) modell letöltése/állapota.
  // Saját motorpéldányon fut (nem cseréli le a chat-modellt). A betöltés után a
  // feltöltött fotót KÖZVETLENÜL megoldja (lásd handleImage → hwSolveImage).
  function setupVisionPanel() {
    var L = window.MagicLLM;
    var loadBtn = $("hw-vision-load");
    if (!loadBtn) return;
    var status = $("hw-vision-status");
    var bar = $("hw-vision-bar"), fill = $("hw-vision-bar-fill");

    if (!L || !L.supported()) {
      loadBtn.disabled = true;
      if (status) status.textContent = "ℹ️ A kép-értő modulhoz WebGPU szükséges (friss Chrome/Edge a számítógépen). Enélkül a kép szövegét OCR olvassa be.";
      return;
    }

    function refresh() {
      var lang = getLang();
      if (L.visionLoading && L.visionLoading()) { loadBtn.textContent = "⏳ Letöltés…"; loadBtn.disabled = true; }
      else if (L.visionReady && L.visionReady()) {
        if (L.langReady(L.VISION_MODEL.id, lang)) { loadBtn.textContent = "✅ " + langDisplay(lang) + " – betöltve"; loadBtn.disabled = true; }
        else { loadBtn.textContent = "⬇️ " + langDisplay(lang) + " letöltése"; loadBtn.disabled = false; }
      } else {
        loadBtn.textContent = "⬇️ Kép-értő modell betöltése · " + modelSize(L.VISION_MODEL);
        loadBtn.disabled = false;
      }
      renderVisionLangs();
    }
    visionUI = { refresh: refresh };

    loadBtn.addEventListener("click", function () {
      var lang = getLang();
      // Már betöltve → ha az aktuális nyelv még nincs „letöltve", gyors aktiválás.
      if (L.visionReady && L.visionReady()) {
        if (L.markLangReady && !L.langReady(L.VISION_MODEL.id, lang)) {
          L.markLangReady(L.VISION_MODEL.id, lang);
          if (status) status.textContent = "✅ " + langDisplay(lang) + " letöltve – a kép-értő modul ezen a nyelven old meg.";
        }
        refresh();
        return;
      }
      ...
      loadBtn.disabled = true;
      if (status) status.textContent = "⬇️ A kép-értő modell letöltése… (az első alkalom hosszabb lehet)";
      L.loadVision(function (pr) {
        var pct = Math.round((pr.progress || 0) * 100);
        if (fill) fill.style.width = pct + "%";
        if (status) status.textContent = pr.text || "Betöltés…";
      }).then(function () {
        if (fill) fill.style.width = "100%";
        if (status) status.textContent = "✅ Kész! Mostantól a feltöltött fotót a kép-értő modul oldja meg – a szöveget ÉS az ábrákat is.";
        refresh();
      }).catch(function (err) {
        if (bar) bar.hidden = true;
        if (status) status.textContent = "⚠️ Nem sikerült betölteni: " + (err && err.message ? err.message : "ismeretlen hiba");
        refresh();
      });
    });

    refresh();
    if (L.visionWasLoaded && L.visionWasLoaded() && !(L.visionReady && L.visionReady())) {
      if (status) status.textContent = "ℹ️ A kép-értő modult korábban már letöltötted – kattints a betöltéshez (a böngészőből, gyorsan).";
    }
  }
  ...
  function setupExtras() {
    ...
    function handleImage(file) {
      ...
      reader.onload = function (ev) {
        ...
        var L = window.MagicLLM;
        // 1) Ha a KÉP-ÉRTŐ (vision) modul be van töltve → a fotót KÖZVETLENÜL megoldja
        //    (a szöveget ÉS az ábrákat/rajzokat is értelmezve), a beállított nyelven.
        if (L && L.visionReady && L.visionReady()) {
          hwSolveImage(url);
          return;
        }
        // 2) Egyébként OCR-rel beolvassuk a szöveget a mezőbe (a régi, megbízható út).
        ...
        runOCR(url).then(function (t) {
          var clean = (window.MagicHomework && window.MagicHomework.normalizeOCR)
            ...
            : (t || "").replace(/[ \t]+/g, " ").replace(/\n{3,}/g, "\n\n").trim();
          textEl.value = clean;
          if (clean) hwStatus("✅ Beolvastam! Ellenőrizd/javítsd a szöveget, majd nyomd meg a Megoldás gombot. 💡 Tipp: töltsd le lent a 🖼️ Kép-értő modult, hogy az ábrákat is megértse és magától megoldja.");
  ...
  // ---------- GAME – játék-szakértő extra funkció ----------
  // Bármely játékban segít (Steam, Roblox, konzol, mobil): elakadás, pénzszerzés,
  // szintlépés, leggyorsabb fejlődés. Minden kérdésért Game-pontok járnak
  // (localStorage), amiket a 3.1-es verzióban be lehet majd váltani.
  //
  // A játék-szakértő HÁROM LÁBON áll (a legjobb játék-asszisztensek receptje:
  // a válasz valódi forrásra alapozása = RAG, így nem "hablatyol"):
  //   1. JÁTÉK-FELISMERÉS – tudja, melyik játékról kérdezel, és a követő
  //      kérdéseknél is észben tartja (beszélgetés-kontextus);
  //   2. ELLENŐRZÖTT JEGYZETEK – a legnépszerűbb játékokhoz kézzel összeállított,
  //      megbízható stratégiák (pénz, XP, bossok), ezekből dolgozik először;
  //   3. WIKI-FORRÁS – a játék SAJÁT wikijében (Fandom, MediaWiki API) néz utána
  //      a kérdésnek, és a válaszát a talált szövegre alapozza. Így pl. a GTA-ról
  //      létező küldetéseket és módszereket mond, nem kitalált dolgokat.

  var GAME_POINTS_KEY = "magicai_game_points";
  var GAME_POINTS_PER_ASK = 10;

  function gamePoints() {
    var n = parseInt(localStorage.getItem(GAME_POINTS_KEY) || "0", 10);
    return isNaN(n) ? 0 : n;
  }
  function renderGamePoints() {
    var el = $("game-points");
    if (el) el.textContent = "🏆 " + gamePoints() + " Game-pont";
  }
  function addGamePoints(n) {
    try { localStorage.setItem(GAME_POINTS_KEY, String(gamePoints() + n)); } catch (e) {}
    renderGamePoints();
  }

  // JÁTÉK-ADATBÁZIS: a legnépszerűbb játékok felismeréshez (aliases), a játék
  // saját wikije (Fandom/MediaWiki, CORS-barát origin=* API) és kézzel
  // ellenőrzött, megbízható alapstratégiák (facts). A facts szándékosan rövid:
  // a kis helyi modellnek ez a "földelés", amitől nem talál ki dolgokat.
  var GAME_DB = [
    {
      name: "GTA Online / GTA V", wiki: "gta.fandom.com",
      aliases: ["gta", "gta 5", "gta v", "gta5", "gta online", "grand theft auto"],
      facts: "Pénzszerzés GTA Online-ban (csalás nélkül, a leggyorsabbak): a Cayo Perico heist egyedül is vihető (Kosatka tengeralattjáró kell hozzá) – ez a legjobb szóló kereset; passzív bevétel: Nightclub (a széfje magától termel), Acid Lab és Bunker; az újabb Mansion-frissítés AI-asszisztense 24 órára felgyorsítja az üzletek termelését; mindig figyeld a heti 2-3-szoros pénz eseményeket. Szintlépés (RP): heistek, klub-/ügynökségi küldetések és versenyek adják a legtöbb RP-t. GTA V történetmód: a legtöbb pénzt a Lester-féle merényletküldetések alatti tőzsdézés hozza (érdemes a sztori végére hagyni, nagy tőkével)."
    },
    {
      name: "Minecraft", wiki: "minecraft.fandom.com",
      aliases: ["minecraft", "ender sarkany", "ender dragon"],
      facts: "Ender sárkány: előbb lődd le íjjal az obszidiántornyokon lévő végzetkristályokat (némelyik vasrács mögött van, oda fel kell építkezni), csak utána támadd a sárkányt; amikor a portál fölé ereszkedik, karddal sebezd, repülés közben íjazz; vigyél vizes vödröt (zuhanás ellen), blokkokat és jó felszerelést. Gyémánt: az 1.18+ világokban Y = -59 környékén bányássz. Gyors XP: enderman-farm a Endben, blaze-farm az erődben vagy falusi kereskedés. Netherite: ősi törmelék a Netherben Y = 15 körül."
    },
    {
      name: "Roblox – Blox Fruits", wiki: "blox-fruits.fandom.com",
      aliases: ["blox fruits", "blox fruit", "bloxfruits"],
      facts: "Leggyorsabb szintezés: mindig a szintedhez ajánlott quest NPC küldetéseit csináld (a szigetek szintsorrendben követik egymást), és lépj tovább, amint új quest nyílik; a bossok kiemelten sok XP-t adnak. Beli: bossok és kincsesládák; Fragment: raidek, Sea Beast és eventek. A gyümölcsöt a játékstílusodhoz válaszd, és következetesen ugyanazokat a fő statokat fejleszd."
    },
    {
      name: "Roblox – Adopt Me", wiki: "adoptme.fandom.com",
      aliases: ["adopt me", "adoptme"],
      facts: "Bucks szerzése: teljesítsd a SAJÁT és a kisállataid igényeit is (mindkettő fizet), ezért érdemes több petet párhuzamosan nevelni; lépj be naponta a sorozat-jutalomért. Kereskedésnél óvatosan: a »trust trade« és minden ingyen-ígéret átverés – csak a beépített csereablakban cserélj, és ne add oda semmid előre."
    },
    {
      name: "Fortnite", wiki: "fortnite.fandom.com",
      aliases: ["fortnite"],
      facts: "Több győzelem: érkezz nyugodtabb helyre (ne a legzsúfoltabb POI-ra), gyűjts gyorsan alapfelszerelést, és indulj korán a zónával – a legtöbb vereség rossz rotációból jön; tartsd a magaslati előnyt; Zero Buildben használd a természetes fedezékeket és a mobilitást. XP: a napi/heti questek és az esemény-questek adják a legtöbbet."
    },
    {
      name: "Elden Ring", wiki: "eldenring.fandom.com",
      aliases: ["elden ring", "eldenring"],
      facts: "Ha egy boss megfog: a fegyverfejlesztés (kovácskövek) többet számít, mint a karakterszint; a Vigor legyen elég magas; használj szellem-hamvakat (a Mimic Tear az egyik legerősebb); nézd meg, van-e a bossnak gyengéje (pl. vérzés, fagy); és teljesen jó taktika máshol felfedezni, majd erősebben visszatérni. Rúnafarm a játék későbbi szakaszában: Mohgwyn-palota környéke."
    },
    {
      name: "Brawl Stars", wiki: "brawlstars.fandom.com",
      aliases: ["brawl stars", "brawlstars"],
      facts: "Trófeagyűjtés: kevés fő brawlert fejlessz magas erőszintre (az erőszint sokat számít), a módhoz és pályához illő brawlerrel játssz, 3v3-ban maradj a csapatoddal, és állj le, ha vesztes szériába kerülsz. A Trophy Road, a Starr Road és az eventek jutalmai vásárlás nélkül is szép fejlődést adnak."
    },
    {
      name: "Clash Royale", wiki: "clashroyale.fandom.com",
      aliases: ["clash royale"],
      facts: "Fejlődés: EGY jól bevált paklit játssz és tanuld meg mélyen (a váltogatás visszavet); számold az elixírt (elixír-előny = nyert csata), ne költs túl egy támadásra, védekezésből indíts kontrát; a kártya- és toronyszint miatt a ládák és season-jutalmak rendszeres begyűjtése a leggyorsabb út."
    },
    {
      name: "Roblox (általában)", wiki: "roblox.fandom.com",
      aliases: ["roblox", "robux"],
      facts: "Robuxot CSAK hivatalos úton lehet szerezni (vásárlás, Premium, saját játék/ruha készítése és eladása) – MINDEN »ingyen Robux« oldal, kód vagy generátor átverés, ami a fiókodat lopja el. Fiókbiztonság: kapcsold be a kétlépcsős azonosítást, és soha ne add meg a jelszavad senkinek."
    },
    {
      name: "EA Sports FC / FIFA", wiki: null,
      aliases: ["fifa", "ea fc", "ea sports fc", "fc 24", "fc 25", "fc 26", "fut", "ultimate team"],
      facts: "Ultimate Team: a heti Rivals/Squad Battles jutalmak rendszeres begyűjtése a legjobb fejlődés; ne költs csomagokra – a piacon célzottan venni mindig jobb üzlet; tanulj meg 3-4 hatékony cselt és védekezésben a jockey-t (kézi védés). Meccsben: türelmes felépítés, ne rohanj be minden labdával."
    },
    {
      name: "Rocket League", wiki: "rocketleague.fandom.com",
      aliases: ["rocket league", "rocketleague"],
      facts: "Fejlődés: a kocsikontroll edzéssel jön – napi 10-15 perc training pack és free play többet ér bárminél; tanulj meg rotálni (ne menjen mindenki a labdára), gazdálkodj a boosttal (a kis 12-es padok is számítanak), és célzottan üss, ne csak »bele a labdába«."
    },
    {
      name: "Valorant", wiki: "valorant.fandom.com",
      aliases: ["valorant"],
      facts: "Rangemelkedés: a célkereszt mindig fejmagasságban legyen ott, ahol ellenfél bukkanhat fel; lassíts – a legtöbb halál felesleges rohanásból jön; 1-2 agentet tanulj meg mélyen; gazdálkodj a creddel (a csapat együtt eco-zzon). Napi bemelegítés (Range, Deathmatch) sokat számít."
    },
    {
      name: "Counter-Strike 2", wiki: "counterstrike.fandom.com",
      aliases: ["cs2", "cs go", "csgo", "counter strike"],
      facts: "Fejlődés: célkereszt-pozíció (fejmagasság, a sarkok elé célozz előre), 1-2 fegyver visszarúgásának (spray) megtanulása, alap füst/villanó lineup-ok, és mozgás közben ne lőj (counter-strafe). Aim-gyakorlás: napi deathmatch és aim-gyakorló pályák."
    },
    {
      name: "Terraria", wiki: "terraria.fandom.com",
      aliases: ["terraria"],
      facts: "Boss-elakadás: építs arénát (több platformsor, kempingtűz és szív-lámpás a regenhez), főzz buff-italokat, és a felszerelésed az adott korszak csúcsa legyen – az aréna és a potik szinte mindig többet segítenek, mint az újrapróbálkozás."
    },
    {
      name: "Geometry Dash", wiki: "geometry-dash.fandom.com",
      aliases: ["geometry dash"],
      facts: "Nehéz pálya: Practice Mode-ban, checkpointokkal tanuld meg részenként; a zene ritmusára játssz (a pályák ritmusra vannak tervezve); rövid, gyakori menetekben gyakorolj – a hosszú, frusztrált ülések rontanak a teljesítményen."
    },
    {
      name: "League of Legends", wiki: "leagueoflegends.fandom.com",
      aliases: ["league of legends", "lol", "league"],
      facts: "Fejlődés: a CS (last hit) a legfontosabb alap – 10 percnél 70+ minion a cél; kevés hőst játssz (1-2 fő + 1 tartalék); nézz a minimapra 5-10 másodpercenként; megölt ellenfél után objektívába fordítsd az előnyt (torony, sárkány)."
    }
  ];

  // Melyik játékról szól a kérdés? A normalizált szövegben keressük az aliasokat
  // (szóhatárral, hogy a "lol" ne találjon bele véletlen szavakba).
  function detectGame(text) {
    var low = " " + normalize(text) + " ";
    for (var i = 0; i < GAME_DB.length; i++) {
      var g = GAME_DB[i];
      for (var j = 0; j < g.aliases.length; j++) {
        var a = g.aliases[j].replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        if (new RegExp("(^|[^a-z0-9])" + a + "($|[^a-z0-9])").test(low)) return g;
      }
    }
    return null;
  }

  // Beszélgetés-kontextus a Game modulban: melyik játékról volt szó, és az utolsó
  // néhány kérdés-válasz (így a "és a bossnál mit csináljak?" típusú követő
  // kérdéseket is érti). Új játéknál a kontextus tiszta lappal indul.
  var gameCtx = { game: null, history: [] };

  // Szándék-felismerés → ANGOL wiki-keresőszó (a Fandom wikik angolul íródnak,
  // a magyar kérdésszavakra nem adnának találatot).
  var GAME_INTENTS = [
    { re: /(penz|beli|bucks|zseton|valuta|gold|coin|arany|keress|keresni|kereset|farm|robux|gyemant|gem)/, en: "money" },
    { re: /(szint|xp|level|fejlod|tapasztalat|rang|rank|trofea|trophy)/, en: "leveling experience" },
    { re: /(boss|foellenseg|fonok|legyozz|gyozz|sarkany|dragon)/, en: "boss" },
    { re: /(kuldetes|quest|mission|heist)/, en: "missions" },
    { re: /(fegyver|weapon|targy|item|felszereles|build|pakli|deck)/, en: "equipment" }
  ];

  // Tulajdonnevek kigyűjtése a kérdésből (pl. boss- vagy helynevek: "Cayo Perico",
  // "Mimic Tear") – ezek a legjobb wiki-keresőszavak, mert magyarul is angolul írjuk őket.
  function gameProperNames(text, game) {
    var skip = {};
    if (game) game.aliases.forEach(function (a) {
      a.split(" ").forEach(function (w) { skip[w] = 1; });
    });
    var words = String(text || "").replace(/[„”"?!.,;:()]/g, " ").split(/\s+/).filter(Boolean);
    var out = [];
    for (var i = 1; i < words.length; i++) { // az 1. szó nagybetűje nem jelzés
      // A kötőjeles magyar toldalékot levágjuk ("Online-ban" → "Online", "Cayo-n" → "Cayo"),
      // különben a toldalékos alak menne keresőszónak, és rossz cikket találna.
      var w = words[i].replace(/-(ban|ben|ba|be|bol|ből|böl|rol|ról|ről|röl|hoz|hez|höz|nal|nál|nel|nél|val|vel|ra|re|ig|nak|nek|on|en|ön|ot|et|t)$/i, "");
      if (/^[A-ZÁÉÍÓÖŐÚÜŰ]/.test(w) && !skip[normalize(w).split(" ")[0]]) out.push(w);
      else if (out.length) break; // csak az első összefüggő név-futamot visszük
    }
    return out.slice(0, 4).join(" ");
  }

  function gameWikiQuery(game, text) {
    var names = gameProperNames(text, game);
    if (names) return names;
    var low = normalize(text);
    for (var i = 0; i < GAME_INTENTS.length; i++) {
      if (GAME_INTENTS[i].re.test(low)) return GAME_INTENTS[i].en;
    }
    return null; // nincs jó keresőszó → kihagyjuk a wikit (a jegyzetek így is megvannak)
  }

  // JSON lekérése a wiki API-ról: előbb közvetlenül (a MediaWiki origin=* CORS-t ad),
  // ha az elhal, CORS-proxyn át.
  function gameApiJson(url) {
    return fetchWithTimeout(url, 8000)
      .then(function (r) { if (!r.ok) throw new Error("wiki-hiba"); return r.json(); })
      .catch(function () {
        return fetchViaProxy(url, 8000).then(function (r) { return r.json(); });
      });
  }

  // A játék SAJÁT wikijében keresünk (MediaWiki API). FONTOS: a Fandomon NINCS
  // TextExtracts (prop=extracts ismeretlen!), ezért két lépésben megyünk:
  // (1) list=search → a legjobb cikk címe; (2) action=parse&section=0 → a cikk
  // bevezetője HTML-ben, amiből itt szedjük ki a tiszta szöveget.
  // Visszaad: { title, text, url } vagy elutasít.
  function fetchGameWiki(game, text) {
    if (!game || !game.wiki) return Promise.reject(new Error("nincs wiki"));
    var q = gameWikiQuery(game, text);
    if (!q) return Promise.reject(new Error("nincs keresőszó"));
    var base = "https://" + game.wiki + "/api.php?format=json&origin=*";
    return gameApiJson(base + "&action=query&list=search&srlimit=1&srsearch=" + encodeURIComponent(q))
      .then(function (d) {
        var hits = d && d.query && d.query.search;
        if (!hits || !hits.length) throw new Error("nincs találat");
        var title = hits[0].title;
        return gameApiJson(base + "&action=parse&prop=text&section=0&redirects=1&disablelimitreport=1&page=" +
          encodeURIComponent(title)).then(function (p) {
          var html = p && p.parse && p.parse.text && p.parse.text["*"];
          if (!html) throw new Error("üres cikk");
          var doc = new DOMParser().parseFromString(html, "text/html");
          // Infobox/táblázat/hivatkozás-számok kiszedése, csak a folyószöveg marad
          doc.body.querySelectorAll("table, aside, script, style, sup, figure, .toc, .infobox, .navbox").forEach(function (n) { n.remove(); });
          var txt = doc.body.textContent.replace(/\s+/g, " ").trim();
          if (!txt) throw new Error("üres kivonat");
          if (txt.length > 1100) txt = txt.slice(0, 1100).replace(/\s+\S*$/, "") + "…";
          return {
            title: (p.parse.title || title),
            text: txt,
            url: "https://" + game.wiki + "/wiki/" + encodeURIComponent(String(p.parse.title || title).replace(/ /g, "_"))
          };
        });
      });
  }

  // A játék-szakértő rendszer-prompt (a nyelvi utasítást az ai.js run()-ja teszi elé)
  function gameSystem() {
    return "Te a Magic AI játék-szakértője vagy (gaming coach), aki VALÓBAN ISMERI a játékokat. " +
      "A felhasználó egy VIDEÓJÁTÉKBAN kér segítséget – lehet Steames, Robloxos, konzolos vagy mobilos játék. " +
      "PONTOSSÁG MINDENEK FELETT: ha a kérdéshez ELLENŐRZÖTT JEGYZETEKET vagy WIKI-FORRÁST kapsz, elsősorban AZOKBÓL dolgozz. " +
      "SOHA ne találj ki küldetéseket, tárgyakat, helyeket, neveket, árakat vagy számokat – ha valamit nem tudsz biztosan, mondd meg őszintén, " +
      "és adj helyette általános stratégiát, ami az ilyen típusú játékokban működik. " +
      "Adj KONKRÉT, azonnal használható segítséget: elakadásnál lépésről lépésre; pénznél/valutánál a leggyorsabb kereseti módok sorrendben; " +
      "szintlépésnél a leghatékonyabb XP-stratégia, egészen a legmagasabb szintekig. " +
      "Rövid bevezető után használj számozott lépéseket vagy felsorolást. " +
      "Ha nem derül ki, melyik játékról van szó, először kérdezz vissza EGYETLEN rövid kérdéssel. " +
      "SOHA ne javasolj csalást, hacket, exploitot vagy pénzköltést kötelező megoldásként – tisztességes, a játék szabályai szerinti módszereket adj.";
  }

  // A modellnek küldött, forrásokkal megalapozott üzenet összeállítása.
  function buildGamePrompt(game, ctx, text) {
    var parts = [];
    if (game) parts.push("A kérdés ehhez a játékhoz kapcsolódik: " + game.name + ".");
    if (game && game.facts) {
      parts.push("--- ELLENŐRZÖTT JEGYZETEK a játékról (megbízható) ---\n" + game.facts + "\n--- jegyzetek vége ---");
    }
    if (ctx && ctx.text) {
      parts.push("--- FORRÁS a játék wikijéből (" + (ctx.title || "wiki") + ") ---\n" + ctx.text + "\n--- forrás vége ---");
    }
    if (gameCtx.history.length) {
      var h = gameCtx.history.map(function (p) {
        return "Kérdés: " + p.q + "\nVálaszod röviden: " + p.a;
      }).join("\n\n");
      parts.push("--- A BESZÉLGETÉS ELŐZMÉNYE (csak kontextusnak) ---\n" + h + "\n--- előzmény vége ---");
    }
    parts.push("KÉRDÉS: " + text);
    parts.push("A jegyzetekre és a forrásra támaszkodj, ahol csak lehet. Amit sem ezek, sem biztos tudásod nem támaszt alá, azt NE állítsd.");
    return parts.join("\n\n");
  }

  function gameStatus(msg) { var el = $("game-status"); if (el) el.textContent = msg || ""; }

  function escapeHtml(s) {
    return String(s || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
  }

  function gameAsk(text) {
    text = (text || "").trim();
    var resEl = $("game-result");
    if (!text) { gameStatus("✍️ Írd be, melyik játékban és miben segítsek!"); return; }
    var L = window.MagicLLM;
    if (!L || !L.isReady()) {
      gameStatus("🤖 Ehhez töltsd be egyszer az AI-modellt (fent: 🤖 AI-modell gomb) – utána offline is segítek bármely játékban!");
      return;
    }

    // Játék-felismerés + kontextus: új játéknál tiszta lap, követő kérdésnél
    // (nincs játéknév a szövegben) az előző játék marad érvényben.
    var g = detectGame(text);
    if (g && gameCtx.game && g !== gameCtx.game) gameCtx.history = [];
    if (g) gameCtx.game = g;
    var game = gameCtx.game;

    gameStatus(game
      ? "🎮 " + game.name + " – utánanézek a játék tudástárában…"
      : "🎮 Gondolkodom a legjobb taktikán…");
    resEl.hidden = false;
    resEl.innerHTML = "";

    // Wiki-utánanézés (ha van hozzá jó keresőszó); hibája nem akadály, csak
    // kevesebb lesz a forrás. Utána a megalapozott prompttal streamelünk.
    var lookup = (game && game.wiki)
      ? fetchGameWiki(game, text).catch(function () { return null; })
      : Promise.resolve(null);

    lookup.then(function (ctx) {
      gameStatus("🎮 Gondolkodom a legjobb taktikán…");
      return L.run(gameSystem(), buildGamePrompt(game, ctx, text), function (full) {
        resEl.innerHTML = formatText(full) + ' <span class="hw-ai-caret">▍</span>';
      }, { temperature: 0.4 }).then(function (full) {
        var html = formatText(full);
        if (ctx && ctx.url) {
          html += '<div class="game-src"><a href="' + escapeHtml(ctx.url) + '" target="_blank" rel="noopener">📖 Forrás: ' +
            escapeHtml(ctx.title || "játék-wiki") + '</a></div>';
        }
        resEl.innerHTML = html;
        gameCtx.history.push({ q: text, a: full.replace(/\s+/g, " ").slice(0, 280) });
        if (gameCtx.history.length > 3) gameCtx.history.shift();
        addGamePoints(GAME_POINTS_PER_ASK);
        gameStatus("🏆 +" + GAME_POINTS_PER_ASK + " Game-pont! Kérdezz csak – minél többet kérdezel, annál több pontod lesz!");
      });
    }).catch(function () {
      gameStatus("😕 Az AI-modell most nem válaszolt – próbáld újra egy kicsit később!");
    });
  }

  function setupGame() {
    var openBtn = $("extra-game");
    var modal = $("game-modal");
    if (!openBtn || !modal) return;
    var closeBtn = $("game-close");
    var backdrop = modal.querySelector(".hw-backdrop");
    var textEl = $("game-text");
    var askBtn = $("game-ask");
    ...
    if (askBtn) askBtn.addEventListener("click", function () { gameAsk(textEl.value); });
    if (textEl) textEl.addEventListener("keydown", function (e) {
      if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); gameAsk(textEl.value); }
    });

    // Példa-chipek: kitöltik a mezőt és rögtön kérdeznek is
    modal.querySelectorAll(".hw-chip").forEach(function (chip) {
      chip.addEventListener("click", function () {
        textEl.value = chip.textContent;
        gameAsk(textEl.value);
      });
    });
  ...
  function enterApp() {
    ...
    autoUpdateCheck();
    if (inputEl) inputEl.focus();
    // Első megnyitáskor: „Személyre szabjuk az élményedet" varázsló (egyszer)
    setTimeout(function () { openOnboarding(false); }, 600);
  }
  ...
  function renderAdmin() {
    ...
    users.forEach(function (u) {
      ...
      var mailLine = u.email
        ...
        : "<em>nincs e-mail</em>";
      html += '<div class="admin-card' + (isAdm ? " is-admin" : "") + (ban ? " banned" : "") + '" data-key="' + escAttr(u.key) + '" data-email="' + escAttr(u.email || "") +
  ...
  // ---------- SZEMÉLYRE SZABÁS – „Személyre szabjuk az élményedet" ----------
  // Első megnyitáskor (belépés után) rövid varázsló: téma, nyelv, beszédstílus,
  // mire fogod használni, honnan hallottál rólunk. Minden választás AZONNAL
  // érvénybe lép (téma/nyelv/stílus), az eredmény a böngészőben tárolódik, és a
  // Beállításokból bármikor újrafuttatható. A beszédstílust az ai.js (MagicLLM)
  // minden AI-prompthoz hozzáteszi, így a chat, a házi és a Game is úgy beszél.

  var ONBOARD_KEY = "magicai_onboard_v1";
  var USECASE_KEY = "magicai_usecase";
  var REFERRAL_KEY = "magicai_referral";

  // A varázsló szövegei: magyar teljes + angol tartalék (a többi nyelv az angolt kapja)
  var OB_T = {
    hu: {
      title: "✨ Személyre szabjuk az élményedet",
      intro: "Néhány gyors kérdés – fél perc az egész –, és a Magic AI pontosan olyan lesz, amilyennek te szeretnéd. 🪄",
      start: "Kezdjük! ✨", next: "Tovább →", back: "← Vissza", done: "Kezdés 🚀", skip: "Kihagyás ⏭",
      stepTheme: "🎨 Milyen témát szeretnél?",
      themeLight: "☀️ Világos", themeLightD: "Ragyogó, nappali felület.",
      themeDark: "🌙 Sötét", themeDarkD: "Kellemes a szemnek, este is.",
      stepLang: "🌐 Milyen nyelven használod?",
      langD: "Az AI ezen a nyelven válaszol, és a felület is erre vált.",
      stepStyle: "🎭 Hogyan beszéljen veled a Magic AI?",
      styleFriendly: "😊 Barátságosan", styleFriendlyD: "Közvetlen, bátorító hangnem – mint egy jó barát.",
      styleConcise: "⚡ Tömören", styleConciseD: "Csak a lényeg, felesleges körítés nélkül.",
      styleDetailed: "📚 Részletesen", styleDetailedD: "Türelmes magyarázat példákkal, lépésről lépésre.",
      styleFunny: "😄 Viccesen", styleFunnyD: "Játékos, humoros válaszok – pontos tartalommal.",
      stepUse: "🚀 Mire fogod használni? (többet is jelölhetsz)",
      useLearn: "📖 Tanulás, házi feladat", useGame: "🎮 Játékok", useWrite: "✍️ Írás, fogalmazás", useAll: "🌈 Minden egyben",
      stepRef: "📣 Honnan hallottál a Magic AI-ról?",
      refFriend: "👥 Barátoktól", refYt: "📺 YouTube", refTt: "🎵 TikTok", refSchool: "🏫 Iskolából", refOther: "✨ Máshonnan",
      doneTitle: "🎉 Kész is!",
      doneText: "A Magic AI mostantól a te ízlésed szerint néz ki és beszél. Bármikor módosíthatsz a ⚙️ Beállítások fülön – jó varázslást! 🪄"
    },
    en: {
      title: "✨ Let's personalize your experience",
      intro: "A few quick questions – half a minute – and Magic AI will be exactly the way you like it. 🪄",
      start: "Let's go! ✨", next: "Next →", back: "← Back", done: "Start 🚀", skip: "Skip ⏭",
      stepTheme: "🎨 Which theme do you prefer?",
      themeLight: "☀️ Light", themeLightD: "Bright, daytime look.",
      themeDark: "🌙 Dark", themeDarkD: "Easy on the eyes, day and night.",
      stepLang: "🌐 Which language will you use?",
      langD: "The AI answers in this language and the interface switches too.",
      stepStyle: "🎭 How should Magic AI talk to you?",
      styleFriendly: "😊 Friendly", styleFriendlyD: "Warm, encouraging tone – like a good friend.",
      styleConcise: "⚡ Concise", styleConciseD: "Just the point, no fluff.",
      styleDetailed: "📚 Detailed", styleDetailedD: "Patient explanations with examples, step by step.",
      styleFunny: "😄 Funny", styleFunnyD: "Playful, humorous answers – with accurate content.",
      stepUse: "🚀 What will you use it for? (pick any)",
      useLearn: "📖 Learning & homework", useGame: "🎮 Games", useWrite: "✍️ Writing", useAll: "🌈 Everything",
      stepRef: "📣 Where did you hear about Magic AI?",
      refFriend: "👥 From friends", refYt: "📺 YouTube", refTt: "🎵 TikTok", refSchool: "🏫 School", refOther: "✨ Somewhere else",
      doneTitle: "🎉 All set!",
      doneText: "Magic AI now looks and talks the way you like. You can change anything later in ⚙️ Settings – happy magic! 🪄"
    }
  };
  function obT() {
    var l = (window.MagicLLM && window.MagicLLM.getLang) ? window.MagicLLM.getLang() : "hu";
    return OB_T[l] || OB_T.en;
  }

  var OB_STEPS = ["intro", "theme", "lang", "style", "use", "ref", "done"];
  var obState = null;

  function obSelectedTheme() { return document.body.classList.contains("dark") ? "dark" : "light"; }

  // Egy választó-kártya HTML-je
  function obOpt(kind, value, title, desc, selected) {
    return '<button type="button" class="ob-opt' + (selected ? " sel" : "") + '" data-kind="' + kind +
      '" data-value="' + value + '"><span class="ob-opt-title">' + title + '</span>' +
      (desc ? '<span class="ob-opt-desc">' + desc + '</span>' : "") + "</button>";
  }
  ...
  function obRender() {
    ...
    var t = obT();
    var body = $("ob-body");
    var step = OB_STEPS[obState.step];
    var html = "";

    if (step === "intro") {
      html = '<div class="ob-hero">🪄</div><h2 class="ob-title">' + t.title + '</h2>' +
        '<p class="ob-sub">' + t.intro + "</p>";
    } else if (step === "theme") {
      var th = obSelectedTheme();
      html = '<h2 class="ob-title">' + t.stepTheme + '</h2><div class="ob-grid">' +
        obOpt("theme", "light", t.themeLight, t.themeLightD, th === "light") +
        obOpt("theme", "dark", t.themeDark, t.themeDarkD, th === "dark") + "</div>";
    } else if (step === "lang") {
      var cur = (window.MagicLLM && window.MagicLLM.getLang) ? window.MagicLLM.getLang() : "hu";
      html = '<h2 class="ob-title">' + t.stepLang + '</h2><p class="ob-sub">' + t.langD + '</p><div class="ob-grid ob-langs">';
      Object.keys(LANG_META).forEach(function (code) {
        html += obOpt("lang", code, langDisplay(code), "", code === cur);
      });
      html += "</div>";
    } else if (step === "style") {
      var st = (window.MagicLLM && window.MagicLLM.getStyle) ? window.MagicLLM.getStyle() : "friendly";
      html = '<h2 class="ob-title">' + t.stepStyle + '</h2><div class="ob-grid">' +
        obOpt("style", "friendly", t.styleFriendly, t.styleFriendlyD, st === "friendly") +
        obOpt("style", "concise", t.styleConcise, t.styleConciseD, st === "concise") +
        obOpt("style", "detailed", t.styleDetailed, t.styleDetailedD, st === "detailed") +
        obOpt("style", "funny", t.styleFunny, t.styleFunnyD, st === "funny") + "</div>";
    } else if (step === "use") {
      html = '<h2 class="ob-title">' + t.stepUse + '</h2><div class="ob-grid">' +
        obOpt("use", "learn", t.useLearn, "", obState.uses.learn) +
        obOpt("use", "game", t.useGame, "", obState.uses.game) +
        obOpt("use", "write", t.useWrite, "", obState.uses.write) +
        obOpt("use", "all", t.useAll, "", obState.uses.all) + "</div>";
    } else if (step === "ref") {
      html = '<h2 class="ob-title">' + t.stepRef + '</h2><div class="ob-grid">' +
        obOpt("ref", "friend", t.refFriend, "", obState.ref === "friend") +
        obOpt("ref", "youtube", t.refYt, "", obState.ref === "youtube") +
        obOpt("ref", "tiktok", t.refTt, "", obState.ref === "tiktok") +
        obOpt("ref", "school", t.refSchool, "", obState.ref === "school") +
        obOpt("ref", "other", t.refOther, "", obState.ref === "other") + "</div>";
    } else if (step === "done") {
      html = '<div class="ob-hero">🎉</div><h2 class="ob-title">' + t.doneTitle + '</h2>' +
        '<p class="ob-sub">' + t.doneText + "</p>";
    }
    if (body) body.innerHTML = html;

    // Haladás-pöttyök + navigációs gombok feliratai
    var prog = $("ob-progress");
    if (prog) {
      var dots = "";
      for (var i = 0; i < OB_STEPS.length; i++) {
        dots += '<span class="ob-dot' + (i <= obState.step ? " on" : "") + '"></span>';
      }
      prog.innerHTML = dots;
    }
    var back = $("ob-back"), next = $("ob-next"), skip = $("ob-skip");
    if (back) back.hidden = obState.step === 0;
    if (next) next.textContent = step === "intro" ? t.start : (step === "done" ? t.done : t.next);
    if (skip) { skip.textContent = t.skip; skip.hidden = step === "done"; }

    // Kártya-kattintások bekötése (minden renderelés után újra)
    if (body) body.querySelectorAll(".ob-opt").forEach(function (btn) {
      btn.addEventListener("click", function () { obPick(btn.getAttribute("data-kind"), btn.getAttribute("data-value")); });
    });
  }

  // Egy opció kiválasztása – a téma/nyelv/stílus AZONNAL érvénybe lép
  function obPick(kind, value) {
    if (kind === "theme") {
      setTheme(value);
    } else if (kind === "lang") {
      var sel = $("lang-select");
      ...
      } else if (window.MagicLLM && window.MagicLLM.setLang) {
        window.MagicLLM.setLang(value);
        applyLang(value);
      }
    } else if (kind === "style") {
      if (window.MagicLLM && window.MagicLLM.setStyle) window.MagicLLM.setStyle(value);
    } else if (kind === "use") {
      obState.uses[value] = !obState.uses[value]; // több is jelölhető
    } else if (kind === "ref") {
      obState.ref = value;
    }
    obRender();
  ...
  function obFinish() {
    try {
      var uses = Object.keys(obState.uses).filter(function (k) { return obState.uses[k]; });
      if (uses.length) localStorage.setItem(USECASE_KEY, uses.join(","));
      if (obState.ref) localStorage.setItem(REFERRAL_KEY, obState.ref);
      localStorage.setItem(ONBOARD_KEY, "1");
    } catch (e) {}
    var screen = $("onboard-screen");
    if (screen) screen.hidden = true;
    document.body.classList.remove("hw-open");
    if (typeof islandFlash === "function") islandFlash("✨", "Személyre szabva! 🪄", 2400);
  }
  ...
  function setupOnboarding() {
    ...
    if (next) next.addEventListener("click", function () {
      if (!obState) return;
      if (obState.step >= OB_STEPS.length - 1) { obFinish(); return; }
      obState.step++;
      obRender();
    });
    if (back) back.addEventListener("click", function () {
      if (!obState || obState.step === 0) return;
      obState.step--;
      obRender();
    });
    if (skip) skip.addEventListener("click", obFinish);
    // Beállítások → „Személyre szabás újra" gomb
    var rerun = $("onboard-rerun");
    if (rerun) rerun.addEventListener("click", function () { openOnboarding(true); });
  }

  // ---------- BEMUTATÓ VIDEÓ (első indításkor) ----------
  // A LEGELSŐ indításkor a Magic AI 3.0 TRAILER (trailer/index.html – a Claude
  // Designból importált 30 mp-es, zenés promó) játszik le teljes képernyőn a
  // splash helyett. Egy teljes lejátszás után magától továbbenged, de a
  // Kihagyás gombbal bármikor tovább lehet lépni. Utána soha többé nem jön elő
  // (INTRO_KEY a localStorage-ban – új kulcs, hogy az ÚJ trailert mindenki
  // egyszer lássa, az is, aki a régi bemutatót már látta).
  // A trailer React-et (unpkg) és Google Fonts-ot tölt a netről, ezért
  // file://-ról vagy net nélkül a régi, teljesen beágyazott bemutato.html megy.

  var INTRO_KEY = "magicai_intro_seen_v2";
  var INTRO_MS = 31000; // a trailer 30 mp-es + kis ráhagyás

  function runIntroOrSplash() {
    var seen = null;
    try { seen = localStorage.getItem(INTRO_KEY); } catch (e) {}
    if (seen) { runSplash(); return; }

    // az első indításnál a normál splash kimarad – rögtön a bemutató jön
    var s = $("splash");
    if (s && s.parentNode) s.parentNode.removeChild(s);

    // A trailer lejátszófeje a localStorage-ban perzisztál ('animstage:t') –
    // nullázzuk, hogy mindig az elejéről induljon.
    try { localStorage.setItem("animstage:t", "0"); } catch (e) {}

    var offline = (typeof navigator !== "undefined" && navigator.onLine === false);
    var src = (location.protocol === "file:" || offline) ? "bemutato.html" : "trailer/index.html";

    var wrap = document.createElement("div");
    wrap.className = "intro-overlay";
    wrap.innerHTML =
      '<iframe class="intro-frame" src="' + src + '" title="Magic AI bemutató"></iframe>' +
      '<button class="intro-skip" type="button">Kihagyás ⏭</button>';
    ...
    setTimeout(finish, INTRO_MS); // egy teljes lejátszás után magától tovább
    wrap.querySelector(".intro-skip").addEventListener("click", finish);
  }

  // ---------- INDÍTÁS ----------

  document.addEventListener("DOMContentLoaded", function () {
    runIntroOrSplash(); // első indításkor a bemutató videó, utána a szokásos splash
    ...
    setupAdmin();
    setupBanModal();
    setupPowerSaver();   // automatikus nyugalmi mód (GPU-kímélés tétlenségkor/háttérben)
    updateStats();
