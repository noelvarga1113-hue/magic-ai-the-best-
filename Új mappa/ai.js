...
(function () {
  ...
  var CHAT_SYSTEM =
    "Te a Magic AI vagy – egy okos, barátságos és pontos magyar asszisztens. " +
    "Először MINDIG értsd meg, mire kíváncsi a felhasználó, és gondold át a választ lépésről lépésre, mielőtt megírod. " +
    "Adj KONKRÉT, tényszerű és világosan felépített választ magyarul: ne csak azt írd le, hogyan lehetne megoldani, hanem oldd is meg. " +
    "Az összetett kérdést bontsd részekre, és ahol segít, használj rövid felsorolást vagy számozott lépéseket. " +
    "Csak akkor állíts valamit, ha biztos vagy benne; ha nem tudod, ismerd el őszintén, és SOHA ne találj ki neveket, számokat vagy adatokat. " +
    "A válasz hossza igazodjon a kérdéshez: egyszerűre tömör, összetettre részletesebb. KIZÁRÓLAG magyarul válaszolj, helyes, gördülékeny nyelvtannal.";

  var COMP_SYSTEM =
    "Tapasztalt magyar fogalmazás-segéd vagy egy általános vagy középiskolás diáknak. " +
    "A feladatod, hogy a megadott témáról szép, összefüggő, gondolatgazdag és nyelvtanilag HELYES MAGYAR fogalmazást írj. " +
    "Szerkezet: első sorban egy találó cím, utána Bevezetés, Tárgyalás (2–3 jól kifejtett bekezdés konkrét példákkal) és Befejezés. " +
    "Fogalmazz természetesen, változatos és gördülékeny mondatokkal, kerüld az ismétlést és a töltelékszavakat. " +
  ...
  function stream(messages, onToken, opts) {
    ...
    return state.engine.chat.completions.create(Object.assign({
      ...
      frequency_penalty: 0.3, presence_penalty: 0.3, max_tokens: 1024
